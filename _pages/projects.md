...
projects:
  ...
  - id: "project-lssph"
    ...
    contents: |
      ...
      <p>一方，従来の古典的なSPH法には，「流れや変形などによって計算粒子配置が乱れると，その離散化精度（計算精度）が著しく低下する」という重大な欠陥があることが知られています．この精度課題の要因は，古典的なSPH法の空間離散化式の定式化において，計算粒子の一様配置が仮定されていることに起因します．実際の流体シミュレーションでは，各計算ステップにおいて粒子が移動し粒子配置が非一様となるため，古典的なSPH法を用いた計算結果の信頼性については目をつぶってきた現状があります．</p>

      <p>この精度課題を克服するため，1990年代頃から理想的な粒子配置を仮定しない<b>高精度なSPH法</b>が数多く提案されてきました．しかし，手法間の関係性や数学的な類似点・相違点を包括的に整理した研究は乏しく，SPH法のユーザーにとって「個々の問題にどの高精度SPH法を選択し実装すべきか」を適切に判断することは非常に困難な状況にありました．さらに，現在も際限なく"新たな手法"が提案され続けているため，その全貌の理解はより一層難しくなっています．</p>

      <p>このような背景を踏まえ，本研究では，多種多様なSPH法の一般化を試み，SPH法の理論的枠組みを再整備することを目的としました．既存の高精度SPH法の定式化について理論的考察を進める中で，「着目粒子における物理量の空間微分値を周辺粒子の情報から近似するプロセスは，最小二乗法に基づく近似手法と本質的に等価である」という新たな視点に至りました．この着想をもとに従来のSPH法の離散化式を見直した結果，古典的SPH法や各種の高精度SPH法が，<b>最小二乗法をベースとした単一の数学的枠組みとして統一的に説明可能である</b>ことを見出しました．本研究では，この最小二乗法に基づく一般化されたSPH法を「<strong>最小二乗SPH (Least-Squares SPH, LS-SPH) 法</strong>」と呼んでいます．</p>

      <p>一般化された枠組みを構築したことで，無数に存在する高精度SPHモデルの中からある特定のSPHモデルを選択する必要がなくなりました．また，各種SPH法の離散化誤差（打切り誤差）を統一的に評価することが可能となり，SPH法の離散化精度に関する理論基盤を確立しました．</p>

      <p>さらに，開発した最小SPH法の実際の流体計算における妥当性を検証するために，3つの流体ベンチマークテスト（2次元）を実施しました．具体的には，剛体壁境界を有するLid-driven cavity flowおよびTaylor–Green vortex flowに加え，自由表面境界を有するOscillating drop testを実施しました<b>(上記の動画1)</b>．なお，最小二乗SPH法と比較するために，古典的SPH法を用いた計算も同様に実施しました．これら一連の検証を通じて，粒子配置が不規則となる実際の流体シミュレーションでは，(1) 古典的SPH法の精度が著しく低下することを定量的に示すと同時に，(2) 最小二乗SPH法では安定して高い計算精度が維持されることを実証しました．</p>
