...
projects:
  ...
  - id: "project-lssph"
    ...
    media_caption: "最小二乗SPH (LS-SPH) 法を用いた流体ベンチマークテスト．左から順に，Lid-driven cavity flow ($Re=10^3$)，Taylor-Green vortex flow ($Re=10^3$)，Oscillating drop の結果を表す．色はそれぞれ，速度の大きさ，圧力，圧力を表す．"

    summary: |
      従来の古典的SPH法が抱える0次精度（解像度を向上させても数値誤差が減少しない）という課題を根本から克服するため，最小二乗法に基づく一般化されたSPH法「<strong>最小二乗SPH (LS-SPH) 法</strong>」を開発しました．理論的な誤差評価や流体ベンチマークテストの両面から，提案手法の数理的妥当性と高い計算精度を実証しました．

    contents: |
      <p>粒子法は，計算格子（メッシュ）を必要としない<b>メッシュフリー法</b>の一種であり，自由表面の大変形や混相流といった複雑な流体現象を扱う上で極めて有効なシミュレーション手法です．中でも，L.B. Lucy, R.A. Gingold, J.J. Monaghanらによって1977年に開発された <b>Smoothed Particle Hydrodynamics (SPH) 法</b> は，その高い汎用性と実装の柔軟性から，惑星科学の銀河形成や巨大衝突の計算をはじめ，計算工学のダム決壊，津波遡上，土砂災害の予測に至るまで，多岐にわたる分野で幅広く応用されてきました．</p>

      <p>一方，従来の古典的なSPH法には，「流れや変形によって粒子配置が乱れると計算精度が著しく低下する」という重大な欠陥があることが知られています．この精度課題の要因は，古典的SPH法の空間離散化式が，計算粒子の一様配置を仮定して定式化されていることに起因します．そのため，古典的SPH法によるシミュレーションでは，計算結果の定量的な妥当性を十分に担保できないことが根本的な課題となっていました．</p>

      <p>この精度課題を克服するため，1990年代頃から理想的な粒子配置を仮定しない<b>高精度SPH法</b>が数多く提案されてきました．しかし，それらの手法間の関係性や，数学的な類似点・相違点を包括的に整理した研究は乏しく，SPH法のユーザーにとって「個々の問題に対してどの高精度SPH法を選択し実装すべきか」を適切に判断することが困難な状況にありました．</p>

      <p>このような背景を踏まえ，本研究では，多種多様なSPH法の一般化を試み，その統一的な数理基盤を構築することを目的としました．既存の高精度SPH法の定式化について理論的考察を進める中で，「着目粒子における物理量の空間微分値を周辺粒子の情報から近似するプロセスは，最小二乗法に基づく近似手法と本質的に等価である」という新たな視点に至りました．この着想をもとにSPH法の理論展開を再考した結果，古典的SPH法や各種の高精度SPH法が，<b>最小二乗法をベースとした単一の数学的枠組みとして統一的に説明可能である</b>ことを見出しました．本研究では，この最小二乗法に基づく一般化されたSPH法を「<strong>最小二乗SPH (Least-Squares SPH, LS-SPH) 法</strong>」と呼んでいます．</p>

      <p>一般化された枠組みを構築したことで，各種SPH法の離散化誤差（打切り誤差）を統一的に評価することが可能となりました．具体的には，各種SPH法の離散化誤差は，最小二乗法で用いるテイラー展開の展開次数にのみ依存することが分かりました．また，古典的SPH法と最小二乗SPH法の実用上の計算精度を比較するため，代表的な流体ベンチマークテストを複数実施しました．これら一連の検証を通じて，乱れた粒子配置下では古典的SPH法の精度が著しく低下することを定量的に示すと同時に，提案する最小二乗SPH法では安定して高い計算精度が維持されることを実証しました．</p>
